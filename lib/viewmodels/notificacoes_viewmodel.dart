...
class NotificacoesViewModel {
  ...
  Future<void> fetchUnreadNotificacoes() async {
    unreadNotificacoes = notificacoes.where((n) => !n.lida).toList();
  }
  ...
  List<Notificacao> filterNotificacoes({
    ...
    return notificacoes.where((notificacao) {
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        ...
        matchesSearch =
            notificacao.produto_nome.toLowerCase().contains(query) ||
                notificacao.solicitante_nome.toLowerCase().contains(query) ||
                notificacao.solicitante_cargo.toLowerCase().contains(query);
      }
